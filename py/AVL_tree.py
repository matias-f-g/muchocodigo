"""This module displays the basic operations on an AVL tree.
...
NODE_RADIUS = 16
NODE_STROKE = 2
HEIGHTS_RADIUS = NODE_RADIUS // 2
HEIGHTS_DIST = int(NODE_RADIUS * 0.8)
...
HEIGHTS_FONT_SIZE = HEIGHTS_RADIUS + 2
...
def main():
    global FPSCLOCK, DISPLAYSURF, NODE_FONT, HEIGHTS_FONT
    pygame.init()
    FPSCLOCK = pygame.time.Clock()
    DISPLAYSURF = pygame.display.set_mode((SREEN_WIDTH, SCREEN_HEIGHT))
    pygame.display.set_caption('AVL Tree')
    NODE_FONT = pygame.font.Font('freesansbold.ttf', DATA_FONT_SIZE)
    HEIGHTS_FONT = pygame.font.Font('freesansbold.ttf', HEIGHTS_FONT_SIZE)
...
class BinaryNode:
    """The node contains some data, but also its own coordenates, edge_x ant heights"""
    def __init__(self, data, pos_x, pos_y, edge_x, height):
        self.data = data
...
def add_binary_element(t, data, prev_node=None, height=0, pos_x=ROOT_X, pos_y=ROOT_Y, edge_x=EDGE_X):
    """Does the classic insert(), but with animation."""
    if t is None:
        new_node = BinaryNode(data, pos_x, pos_y, edge_x, height)
        if prev_node:
        ...
        if data < t.data:
            t.left = add_binary_element(t.left, data, t, height+1, pos_x-edge_x, pos_y+EDGE_Y, edge_x//2)
        elif t.data < data:
            t.right = add_binary_element(t.right, data, t, height+1, pos_x+edge_x, pos_y+EDGE_Y, edge_x//2)
        else:
...
def balance(t):
    ...
    t = update_heights(t)
    draw_heights(t, NODE_INT_COLOR_H, NODE_EXT_COLOR_H)
    pygame.time.wait(1000)

    # The subtree from the left is bigger
    if (t.heightL - t.heightR) > ALLOWED_IMBALANCE:
        draw_heights(t, RED, WHITE)
        pygame.time.wait(1500)
        if t.left.heightL >= t.left.heightR:
            t = rotateWithLeftChild(t)
        else:
            t = doubleWithLeftChild(t)
        draw_node(t, NODE_INT_COLOR_H, NODE_EXT_COLOR_H, True)
        pygame.time.wait(2000)
    # Else, if the subtree from the right is bigger
    elif (t.heightR - t.heightL) > ALLOWED_IMBALANCE:
        draw_heights(t, RED, WHITE)
        pygame.time.wait(1500)
        if t.right.heightR >= t.right.heightL:
...
def update_heights(t):
    t.heightL = find_height(t.left)
    t.heightR = find_height(t.right)
    t.height = max(t.heightL, t.heightR) + 1
    return t


def find_height(node):
    if node is None:
        return -1 # Base case: Empty tree/subtree
...
def rotateWithLeftChild(k2):
    ...
    k2.left = k1.right
    k1.right = k2
    k2 = update_heights(k2)
    k1 = update_heights(k1)
    return k1
...
def draw_node(node, color_int=NODE_INT_COLOR, color_ext=NODE_EXT_COLOR, update_and_wait=True):
    ...
    # The heights of the subtrees of the node
    draw_heights(node, color_int, color_ext, update_and_wait)


def draw_heights(node, color_int, color_ext, update_and_wait=True):
    pos_x, pos_y = node.coords
    pygame.draw.circle(DISPLAYSURF, color_int, (pos_x-HEIGHTS_DIST, pos_y+HEIGHTS_DIST), HEIGHTS_RADIUS)
    pygame.draw.circle(DISPLAYSURF, color_ext, (pos_x-HEIGHTS_DIST, pos_y+HEIGHTS_DIST), HEIGHTS_RADIUS, 1)
    hightL_surf = HEIGHTS_FONT.render(str(node.heightL), True, WHITE)
    DISPLAYSURF.blit(hightL_surf, hightL_surf.get_rect(center=(pos_x-HEIGHTS_DIST, pos_y+HEIGHTS_DIST)))

    pygame.draw.circle(DISPLAYSURF, color_int, (pos_x+HEIGHTS_DIST, pos_y+HEIGHTS_DIST), HEIGHTS_RADIUS)
    pygame.draw.circle(DISPLAYSURF, color_ext, (pos_x+HEIGHTS_DIST, pos_y+HEIGHTS_DIST), HEIGHTS_RADIUS, 1)
    hightR_surf = HEIGHTS_FONT.render(str(node.heightR), True, WHITE)
    DISPLAYSURF.blit(hightR_surf, hightR_surf.get_rect(center=(pos_x+HEIGHTS_DIST, pos_y+HEIGHTS_DIST)))
...
def create_tree():
    ...
    title_surf = title_font.render("Type numbers to build your AVL tree", True, WHITE)
